...
export default function EstadisticasReportPage() {
  ...
  const reportTitle =
    REPORT_OPTIONS.find((opt) => opt.value === reportType)?.label || 'Reporte';

  const dateColumnTitle =
    reportType.includes('inasistencia') || reportType.includes('faltas')
      ? `Fecha de Falla`
      : `Fecha de Registro`;

  // --- Filtering Logic ---
  const filteredData = useMemo(() => {
    if (!reporteData || !despacho) return [];

    return reporteData.filter((item: Asistencia) => {
      // Employee Filter
      ...
      // Supervisor Despacho Filter
      if (!item.despacho || item.despacho.id !== despacho.id) {
        return false;
      }

      return true;
    });
  }, [reporteData, empleadoFilter, despacho]);
  ...
  // --- Handle despacho loading/error before showing table ---
  if (isLoadingDespacho) {
    return (
      <div className="flex justify-center items-center h-64 text-gray-500">
        <Loader className="animate-spin" /> Cargando despacho del supervisor...
      </div>
    );
  }
  ...
  if (isErrorDespacho) {
    return (
      <div className="flex justify-center items-center h-64 text-red-600">
        <AlertTriangle /> Error al cargar el despacho del supervisor: {errorDespacho.message}
      </div>
    );
  }
      ...
      <div className="container mx-auto px-2">
        <main className="max-w-7xl mx-auto mt-8 p-6 bg-white rounded-lg shadow-lg">
          <h1 className="text-2xl font-bold text-gray-800 border-b border-gray-200 pb-3 mb-6">
            Reporte de {reportTitle} — Despacho: {despacho?.nombre}
          </h1>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 mb-6 p-4 border rounded-lg bg-gray-50">
            ...
            <div className="flex items-end col-span-1 md:col-span-2 lg:col-span-4 justify-end">
              <button
                onClick={clearFilters}
                className="w-full md:w-auto px-4 py-2 rounded text-white bg-gray-500 hover:bg-gray-600 transition"
              >
                Limpiar Filtros
              </button>
            </div>
